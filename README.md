...
### 🔐 Безопасность
- **Хранение авторизации** — никнейм пользователя сохраняется в `data/data.json`
- **Безопасный запуск** — игровые процессы запускаются через нативный WinAPI `ShellExecuteW`
- **Изоляция данных** — конфигурация и пользовательские данные хранятся локально в отдельной директории `data/`
