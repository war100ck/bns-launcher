...
async fn get_auth_data() -> Result<Option<String>, String> {
    ...
    let enc = tokio::fs::read_to_string(&path)
        .await
        .map_err(|e| e.to_string())?;
    let dec = decrypt(&enc)?;
    let val: serde_json::Value =
        serde_json::from_str(&dec).map_err(|e| e.to_string())?;
...
async fn save_auth_data(nickname: String) -> Result<(), String> {
    ...
    tokio::fs::write(get_auth_path(), enc)
        .await
        .map_err(|e| e.to_string())
}

// ИСПРАВЛЕНО: возвращаем ключи вместо русских строк — перевод на фронте
#[tauri::command]
async fn get_game_version(architecture: String) -> Result<String, String> {
    let ini = get_bin_dir(&architecture).join("Version.ini");
    if !ini.exists() {
        eprintln!("[VERSION] File not found: {:?}", ini);
        return Ok("NOT_FOUND".to_string());
    }
    let content = tokio::fs::read_to_string(&ini)
        .await
        .map_err(|e| e.to_string())?;
...
async fn check_connection(server_ip: String) -> Result<bool, String> {
    match timeout(
        ...
        TcpStream::connect(format!("{}:3000", server_ip)),
    )
    .await
    {
        Ok(Ok(_)) => Ok(true),
...
#[tauri::command]
async fn fetch_launcher_data(server_ip: String) -> Result<serde_json::Value, String> {
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(2))
        .build()
        .map_err(|e| e.to_string())?;
    client
        .get(&format!("http://{}:3000/api/launcher-data", server_ip))
        .send()
        .await
        .map_err(|e| e.to_string())?
        .json()
...
fn launch_game(architecture: String, server_ip: String) -> Result<(), String> {
    ...
    if res as isize <= 32 {
        Err(format!("Launch error (code: {})", res as isize))
    } else {
        Ok(())
    }
}
...
async fn signup(
    ...
    let client = reqwest::Client::builder()
        ...
        .map_err(|e| e.to_string())?;
    let resp = client
        .post(&format!("http://{}:3000/signup", server_ip))
...
async fn signin(
    ...
    let client = reqwest::Client::builder()
        ...
        .map_err(|e| e.to_string())?;
    let resp = client
        .post(&format!("http://{}:3000/signin", server_ip))
        .json(&serde_json::json!({
            "signin_email": email,
    ...
    if resp.status() == 200 {
        ...
        Err(format!("Login error: {}", resp.status()))
    }
}
